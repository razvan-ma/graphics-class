...
void main()
{
	...
	else if(type_of_light == 2)
	{
		...
		float specularIntensity = pow(max(dot(N, H),0) * lightIntensity, material_shininess * material_shininess);
		float specularIntensitySmooth = smoothstep(0.005, 0.01, specularIntensity) * material_ks;
		light = ambientLight + lightIntensity + specularIntensitySmooth;
